...
namespace NitroSystem.Dnn.BusinessEngine.Api.Controller
{
    ...
    public class StudioController : DnnApiController
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public HttpResponseMessage UploadExtensionPackage()
        {
            try
            {
                string result = "";
                ...
                if (Request.Content.IsMimeMultipartContent())
                {
                    ...
                    Task.Factory
                    .StartNew(() => Request.Content.ReadAsMultipartAsync(streamProvider).Result,
                    ...
                    var tempPath = this.PortalSettings.HomeSystemDirectoryMapPath + @"BusinessEngine\Temp\" + scenarioName + @"\" + Path.GetFileNameWithoutExtension(filename);
                    FastZip fastZip = new FastZip();
                    fastZip.ExtractZip(filename, tempPath, null); //Will always overwrite if target filenames already exist

                    var files = Directory.GetFiles(tempPath);
                    var manifestFile = files.FirstOrDefault(f => Path.GetFileName(f) == "manifest.json");
                    if (!string.IsNullOrEmpty(manifestFile))
                    {
                        string json = FileUtil.GetFileContent(manifestFile);
                        result = json;// JsonConvert.DeserializeObject<ExtensionManifestInfo>(json);
                        //result.TempFolderPath = tempPath;
                        tempPath1 = tempPath;
                    }
                ...
                return Request.CreateResponse(HttpStatusCode.OK, new { Extension = result, TempPath = tempPath1 });
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<HttpResponseMessage> InstallExtension([FromUri] string extensionFile)
        {
            try
            {
                var files = Directory.GetFiles(extensionFile);
                var manifestFile = files.FirstOrDefault(f => Path.GetFileName(f) == "manifest.json");
                string json = FileUtil.GetFileContent(manifestFile);
                ExtensionManifestInfo extension = JsonConvert.DeserializeObject<ExtensionManifestInfo>(json);

                var scenarioID = Guid.Parse(Request.Headers.GetValues("ScenarioID").First());

                var extensionController = new ExtensionService(extension, scenarioID, this.PortalSettings, this.UserInfo);
                await extensionController.InstallExtension();
